...
log = logging.getLogger("red.sinbadcogs.globalban")
...
for handler in log.handlers:
    ...
    fp = cog_data_path(raw_name="GlobalBan") / "unhandled_exceptions.log"
...
class GlobalBan(commands.Cog):
    ...
    @commands.group()
    async def gbanset(self, ctx: commands.Context):
        """
        Options for globalban
        """
        pass
    ...
    async def mban(self, ctx: commands.Context, *ids: int):
        """
    ...
    async def ban_or_hackban(
        ...
        *,
        ...
        reason = reason or "Global Ban Action"
        if member is None:
        ...
        *,
        ...
        for target in targets:
            if await self.ban_filter(guild, mod, target):
                await self.ban_or_hackban(
                    guild, target.id, mod=mod, reason="Global Ban Action"
                )
                if artificial_delay:
